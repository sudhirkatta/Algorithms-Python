...
class Solution:
    ...
    def nSum_rec(self, nums: list[int]) -> list[list[int]]:
        ...
        self.result = []
        locallist = []
        self.rec_sum(locallist=locallist, lidx=0,
                     targetsum=self.targetsum, nsum=self.nsum, previous=None)
        return self.result
    ...
    def rec_sum(self, locallist, lidx, targetsum, nsum, previous):
        ...
        r = len(self.nums)-1
        print(
            f"-:   {remain_targetsum}; {locallist} {self.nums[l:]}, {nsum}, [{l} {r}], {previous}")
        if nsum == 2:
            l_prev_val = None
            r_prev_val = None
            ...
            while (l < r):
                ...
                if twosum == remain_targetsum:
                    print(f"---> {remain_targetsum}; {locallist},{self.nums[l]}, {self.nums[r]}, {previous}")
        ...
        if lidx <= (len(self.nums) - nsum):
            print(f"second rec call")
            self.rec_sum(locallist, lidx+1, targetsum, nsum, previous)
